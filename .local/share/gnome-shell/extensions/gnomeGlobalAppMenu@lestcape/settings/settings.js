...
XletSettingsBase.prototype = {
    ...
    _ensureSettingsFiles: function() {
        ...
        let xletDir = Gio.file_new_for_path(EXTENSION_PATH);//this.ext_type.maps.dirs[this.uuid];
    ...
    _loadFromFile: function() {
        let rawData = Shell.get_file_contents_utf8_sync(this.file.get_path());
    ...
    _saveToFile: function() {
        ...
        Shell.write_string_to_stream(out_file, rawData);
        out_file.close(null);
        this.monitorId = this.monitor.connect("changed", Lang.bind(this, this._checkSettings));
    },

    // called by ShellDBus.js to when the setting is changed remotely. This is to expedite the
...
AppletSettings.prototype = {
    ...
    _get_is_multi_instance_xlet: function(uuid) {
        return false;//Extension.get_max_instances(uuid, this.ext_type) != 1;
    },
};
